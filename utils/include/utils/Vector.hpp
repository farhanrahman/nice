...
public:
  ...
  Vec2f Normalized() const{
    float l = Length();
    if (l > 0) {
        return(Vec2f(data[0] / l, data[1] / l)); }
    else
  ...
  Vec2f operator+(const Vec2f& V) const{
    return Vec2f(data[0]+V.data[0], data[1]+V.data[1]);
  }
  Vec2f operator-(const Vec2f& V) const{
    return Vec2f(data[0]-V.data[0], data[1]-V.data[1]);
  }
  Vec2f operator*(float f) const{
    return Vec2f(data[0]*f, data[1]*f);
  }  
...
public:
  ...
  void Scale(float d0, float d1, float d2) {
    ...
    data[2] *= d2; }
  Vec3f Scale(float s) {
    return Vec3f(data[0]*s,data[1]*s,data[2]*s);
  }    
  ...
  Vec3f Normalized() const{
    float l = Length();
    if (l > 0) {
        return(Vec3f(data[0] / l, data[1] / l,data[2] / l)); }
    else
