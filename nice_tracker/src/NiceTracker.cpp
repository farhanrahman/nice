#include <nice_tracker/NiceTracker.hpp>
#include <pluginlib/class_list_macros.h>

PLUGINLIB_DECLARE_CLASS(nice_tracker, NiceTracker, nice_tracker::NiceTracker, nodelet::Nodelet);

namespace nice_tracker{
	void NiceTracker::onInit(){ 
			...
			// sync.reset(new Synchronizer(MySyncPolicy(32), detect));
			// sync->registerCallback(boost::bind(&NiceTracker::start_tracking, this, _1, _2));
			// sync->registerCallback(boost::bind(&NiceTracker::start_tracking, this, _1));
			...
			running = true;
			kalmanThread = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&NiceTracker::KalmanLoop, this)));
			driverThread = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&NiceTracker::DriverLoop, this)));

			
			predictions = nh->advertiseService("prediction", &NiceTracker::get_prediction, this);
	}

	NiceTracker::~NiceTracker(){
    		if (running){
	...
	bool NiceTracker::get_prediction(nice_detector::prediction::Request& request, nice_detector::prediction::Response& response)
	{
	...
	void NiceTracker::start_tracking (const nice_detector::DetectionListConstPtr& detects/*, const nav_msgs::OdometryConstPtr& odom*/){
		...
		ROS_INFO("start_tracking");

		//If there are no tracks make optimization and automacially create new track for every detection
		if(nrows > 0){
  ...
  /* Publish markers */
	void NiceTracker::Mark_path(double x, double y, double z, int r, int g, int b){ 
	...
	void NiceTracker::create_new_track(const nice_detector::Detection detection, geometry_msgs::PointStampedPtr& worldPoint, double z){
	...
	void NiceTracker::make_command(geometry_msgs::PointStampedPtr& worldPoint){
		/*
	...
	geometry_msgs::PointStamped NiceTracker::getWorldPoint(geometry_msgs::PointStampedPtr& p, const char *frame){
