...
namespace nice_tracker{
	...
	class Track{
		...
		public:	  	
		  ...
		  Track(KalmanStates* kal, nice_detector::Detection det, geometry_msgs::PointStampedPtr& pos):
				kalman(kal), 
				r(std::rand() % 255),
				g(std::rand() % 255),
				b(std::rand() % 255),
		  ...
		  ~Track(){
			...
			void set_detection(nice_detector::Detection det){
				boost::mutex::scoped_lock l(m_mutex);
				detection = det;
			...
			nice_detector::Detection get_detection(){
				boost::mutex::scoped_lock l(m_mutex);
				return detection;
			};
