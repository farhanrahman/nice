...
private:
	...
	nice_detector::Detection *detection;

  public:
    Cluster (float input_height, pcl::PointCloud<PointType>::Ptr& input_cluster, float input_distance, float input_max_y, const std_msgs::Header &headerIn){
	  	height = new float;
	  	distance = new float;
	  	max_y = new float;
		header = new std_msgs::Header(headerIn);
	  	boundingBox = new visualization_msgs::Marker();
	  	detection = new nice_detector::Detection();
	...
	nice_detector::Detection* Create_msg(void);
