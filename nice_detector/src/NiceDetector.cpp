#include <nice_detector/NiceDetector.hpp>
...
PLUGINLIB_DECLARE_CLASS(nice_detector, NiceDetector, nice_detector::NiceDetector, nodelet::Nodelet); 

namespace nice_detector
{
/*Constructor*/
void NiceDetector::onInit(){  
  	...
  	//std::cout << "Setting parameters from launch file:" << std::endl;
		...
		nh->param("NiceDetector/downsample_leaf_size", DOWNSAMPLING_LEAF_SIZE, 0.06);
		nh->param("NiceDetector/ransac_distance_threshold", RANSAC_DISTANCE_THRESHOLD, 0.06);
		nh->param("NiceDetector/ransac_max_iterations", RANSAC_MAX_ITERATIONS, 250);
		nh->param("NiceDetector/cluster_tolerance", CLUSTER_TOLERANCE, 0.12);
		nh->param("NiceDetector/max_allowed_cluster_depth", MAX_ALLOWED_CLUSTER_DEPTH, 6.5);
		nh->param("NiceDetector/min_allowed_cluster_depth", MIN_ALLOWED_CLUSTER_DEPTH, 1.3);
		nh->param("NiceDetector/max_allowed_cluster_height", MAX_ALLOWED_CLUSTER_HEIGHT, 2.2);
		nh->param("NiceDetector/min_allowed_cluster_height", MIN_ALLOWED_CLUSTER_HEIGHT, 1.4);
		nh->param("NiceDetector/max_allowed_cluster_width", MAX_ALLOWED_CLUSTER_WIDTH, 0.9);
		nh->param("NiceDetector/min_allowed_cluster_width", MIN_ALLOWED_CLUSTER_WIDTH, 0.5);

		nh->param("NiceDetector/cluster_min_size", CLUSTER_MIN_SIZE, 200);
		nh->param("NiceDetector/cluster_max_size", CLUSTER_MAX_SIZE, 700);
		nh->param("NiceDetector/publish_blobs", PUB_BLOBS, true);
		nh->param("NiceDetector/publish_clusters", PUB_CLUSTERS, false);
		nh->param("NiceDetector/hog_threshold", HOG_THRESHOLD, -0.0);
		nh->param("NiceDetector/adaboost_threshold", ADABOOST_THRESHOLD, 0.0);
		nh->param("NiceDetector/adaboost_search_area", ADABOOST_SEARCH_AREA, 2.0);
		nh->param("NiceDetector/adaboost_init_steps", ADABOOST_INIT_STEPS, 50.0);
		nh->param("NiceDetector/adaboost_num_base_class", ADABOOST_NUM_BASE_CLASS, 100.0);

		nh->param("NiceDetector/area_x", AREA_X, 0.50);
		nh->param("NiceDetector/area_y", AREA_Y, 0.50);
		nh->param("NiceDetector/area_z", AREA_Z, 0.50);

		nh->param("NiceDetector/max_predictions", MAX_PREDICTIONS, 30);
		nh->param("NiceDetector/max_frames_withouth_hog_confirmation", MAX_FRAMES_WITHOUT_HOG_CONFIRMATION, 10);
		...
		sync->registerCallback(boost::bind(&NiceDetector::cloud_preprocessing, this, _1, _2, _3));
		...
		detections = nh->advertise<nice_detector::DetectionList> ("detections",1);
...
nice_detector::prediction::Response NiceDetector::getKalmanPrediction(){
	nice_detector::prediction::Request req;	
  nice_detector::prediction::Response res;	
 ...
 */
void NiceDetector::publish_rgb_blobs(list<Cluster>* clusters, const sensor_msgs::ImageConstPtr& image, const sensor_msgs::CameraInfoConstPtr& cam_info, const std_msgs::Header header){
	//std::cout << "Zacetek publish_rgb_blobs" << std::endl;
	list<Cluster>::iterator it;
	Rect validROI(0, 0, 480,640);
  nice_detector::DetectionList detectionMsg;
	...
	else 
	{
		...
		if(!(clusters->empty()))
		{
			...
			if (best_conf >= ADABOOST_THRESHOLD){
				...
				nice_detector::Size best_trackedPatchSize;
...
void NiceDetector::calculate_search_window(float x, float y, float z)
{
...
void NiceDetector::publish_pc_clusters(list<Cluster>* clusters, std::string frame_id, ros::Time stamp, bool show_box){
	list<Cluster>::iterator it;
...
void NiceDetector::publish_height (float x, float y, float z, std::string msg, std::string frame_id){
 ...
 */
void NiceDetector::cloud_preprocessing (const sensor_msgs::PointCloud2ConstPtr& input, const sensor_msgs::ImageConstPtr& image, const sensor_msgs::CameraInfoConstPtr& cam_info){
	bool data = true;
  ...
  /* If we haven't found or lost human we search the whole area - point cloud */
	...
	if(data){  		
		...
		if(have_new_patch){
			cv::rectangle(im_rgb,
		         cv::Point(patch_left,patch_upper),
		         cv::Point(patch_left+patch_width, patch_upper+patch_height),
		         cv::Scalar( 0, 255, 255 ),
		         3,
		         8 );
  ...
  */
	void NiceDetector::Mark_search_box(){
